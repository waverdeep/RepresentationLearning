...
def main():
    ...
    parser.add_argument('--configuration', required=False,
                        default='./config/config-downstream-WaveBYOLEfficientB7-VoxCeleb-96000-512.json')
    ...
    acoustic_dict = train_dataset.speaker_dict
    format_logger.info("speaker_num: {}".format(len(acoustic_dict.keys())))
    ...
    for epoch in range(num_of_epoch):
        ...
        test_accuracy, test_loss = test(config, writer, epoch, pretext_model, downstream_model,
                                        test_loader, optimizer, format_logger, acoustic_dict)
...
def train(config, writer, epoch, pretext_model, downstream_model, data_loader, optimizer, format_logger, acoustic_dict):
    total_loss = 0.0
    total_accuracy = 0.0
    pretext_model.eval()
    ...
    for batch_idx, (waveform, acoustic_id) in enumerate(data_loader):
        ...
        with torch.no_grad():
            representation, vec = pretext_model.get_projection(data)
        ...
        # if batch_idx % 50 == 0:
        #     latent_space = vec[0].detach()
        #     pre_latent_space = latent_space[0].cpu().numpy()
        #
        #     tensorboard.add_latent_heatmap(writer, pre_latent_space, "TrainPreLatentURBSND",
        #                                    "online01_vs_target02", (epoch - 1) * len(data_loader) + batch_idx)
        #     latent_space = vec[1].detach()
        #     online01_output_rep = latent_space[1].squeeze().cpu().numpy()
        #     tensorboard.add_latent_heatmap(writer, online01_output_rep,
        #                                    "TrainEncoderLatentURBSND",
        #                                    "online01_vs_target02", (epoch - 1) * len(data_loader) + batch_idx)
...
def test(config, writer, epoch, pretext_model, downstream_model, data_loader, optimizer, format_logger, acoustic_dict):
    total_loss = 0.0
    total_accuracy = 0.0
    pretext_model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (waveform, acoustic_id) in enumerate(data_loader):
            ...
            #     pre_latent_space = latent_space[0].cpu().numpy()
            #
            #     tensorboard.add_latent_heatmap(writer, pre_latent_space, "TestPreLatentURBSND",
            #                                    "online01_vs_target02", (epoch - 1) * len(data_loader) + batch_idx)
            #     latent_space = vec[1].detach()
            #     online01_output_rep = latent_space[1].squeeze().cpu().numpy()
            #     tensorboard.add_latent_heatmap(writer, online01_output_rep,
            #                                    "TestEncoderLatentURBSND",
            #                                    "online01_vs_target02", (epoch - 1) * len(data_loader) + batch_idx)

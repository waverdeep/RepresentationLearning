...
class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, stride, filter_size, padding):
        ...
        self.encoder02.add_module(
            ...
            nn.Sequential(
                nn.Conv2d(1, 3, kernel_size=1, stride=1, padding=1),
            )
        )
    def forward(self, x):
    ...
    def __init__(self, vgg_version):
        ...
        if vgg_version == "16":
            self.encoder02.add_module(
                "resnet_encoder_layer",
                nn.Sequential(
                    torchvision.models.vgg16_bn(pretrained=True).features
                )
            )
        elif vgg_version == "19":
            self.encoder02.add_module(
                "vgg_encoder_layer",
                nn.Sequential(
                    torchvision.models.vgg19_bn(pretrained=True).features
                )
            )

    def forward(self, x):
        out = self.encoder02(x)
        return out
...
class WaveBYOL(nn.Module):
    def __init__(self, config, encoder_input_dim, encoder_hidden_dim, encoder_filter_size, encoder_stride, encoder_padding,
                 mlp_input_dim, mlp_hidden_dim, mlp_output_dim, vgg_version):
        ...
        self.feature_extractor = FeatureExtract(vgg_version=vgg_version)
        set_requires_grad(self.feature_extractor, requires=False)
    ...
    def get_representation(self, x):
        online = self.online_encoder_network(x)
        return online

    def get_post_representation(self, x):
        online = self.online_encoder_network(x)
        online_post = self.online_post_network(online)
        return online_post
    ...
    def forward(self, x01, x02):
        ...
        with torch.no_grad():
            online01 = self.feature_extractor(online01)
        ...
        with torch.no_grad():
            online02 = self.feature_extractor(online02)
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                target01 = self.feature_extractor(target01)
            ...
            with torch.no_grad():
                target02 = self.feature_extractor(target02)
...
if __name__ == '__main__':
    test_model = WaveBYOL(
        ...
        encoder_filter_size=[10, 8, 4, 4, 4],
    ...
    print(test_model)
    input_data01 = torch.rand(2, 1, 15200).cuda()

        ...
        self.network = nn.Sequential(
            collections.OrderedDict(
                [
                    ...
                    ("mobilnet_v2_layer02", torchvision.models.mobilenet_v3_large(pretrained=True).features)
                ]
            )
        )
...
if __name__ == '__main__':
    data = torch.rand(8, 3, 512, 128).cuda()
    # mobilenet -> 빠르다라는 장점을 가지고 있음 (그리고 그만큼 성능이 떨어지는 것도 아닌 것으로 알고 있음)
    ...
    online_x01_pre, online_x02_pre, online_representation01_output, online_representation02_output, target_x01_pre, target_x02_pre, target_representation01_output, target_representation02_output, loss = test_model(
        input_data01, input_data02)

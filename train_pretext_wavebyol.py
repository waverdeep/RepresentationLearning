...
def main():
    ...
    parser.add_argument('--configuration', required=False,
                        default='./config/config_pretext-WAVEBYOLTest02-LFMU-resnet152aug-batch64.json')
    ...
    for epoch in range(num_of_epoch):
        ...
        if epoch % 50 == 0:
            train_tool.save_checkpoint(config=config, model=model, optimizer=optimizer,
                                       loss=test_loss, epoch=best_epoch, format_logger=format_logger, mode="step-{}".epoch,
                                       date='{}'.format(now))

...
def main():
    ...
    parser.add_argument('--configuration', required=False,
                        default='./config/config_pretext-WAVEBYOLTest01-librispeech100-resnet50aug-batch64.json')
...
def train(config, writer, epoch, model, train_loader, optimizer):
    ...
    for batch_idx, (waveform01, waveform02, filename, speaker_id) in enumerate(train_loader):
        ...
        online01, online02, target01, target02, loss = model(data01, data02)
        model.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if batch_idx % 50 == 0:
            online01_output = online01.detach()
            online02_output = online02.detach()
            target01_output = target01.detach()
            target02_output = target02.detach()
            online01_output = online01_output[0].permute(1, 0).cpu().numpy()
            online02_output = online02_output[0].permute(1, 0).cpu().numpy()
            target01_output = target01_output[0].permute(1, 0).cpu().numpy()
            target02_output = target02_output[0].permute(1, 0).cpu().numpy()

            # tensorboard.add_latent_heatmap(writer, online01_output, "TrainLatentVector", "online01", (epoch - 1) * len(train_loader) + batch_idx)
            # tensorboard.add_latent_heatmap(writer, online02_output, "TrainLatentVector", "online02", (epoch - 1) * len(train_loader) + batch_idx)
            # tensorboard.add_latent_heatmap(writer, target01_output, "TrainLatentVector", "target01", (epoch - 1) * len(train_loader) + batch_idx)
            # tensorboard.add_latent_heatmap(writer, target02_output, "TrainLatentVector", "target02", (epoch - 1) * len(train_loader) + batch_idx)
            tensorboard.add_byol_latent_heatmap(writer, online01_output, target02_output, "TrainLatentVector", "online01_vs_target02", (epoch - 1) * len(train_loader) + batch_idx)
            tensorboard.add_byol_latent_heatmap(writer, online02_output, target01_output, "TrainLatentVector", "online02_vs_target01", (epoch - 1) * len(train_loader) + batch_idx)
    ...
    with torch.no_grad():
        for batch_idx, (waveform01, waveform02, filename, speaker_id) in enumerate(test_loader):
            ...
            if batch_idx % 50 == 0:
                ...
                # tensorboard.add_latent_heatmap(writer, online01_output, "TestLatentVector", "online01",
                #                                (epoch - 1) * len(test_loader) + batch_idx)
                # tensorboard.add_latent_heatmap(writer, online02_output, "TestLatentVector", "online02",
                #                                (epoch - 1) * len(test_loader) + batch_idx)
                # tensorboard.add_latent_heatmap(writer, target01_output, "TestLatentVector", "target01",
                #                                (epoch - 1) * len(test_loader) + batch_idx)
                # tensorboard.add_latent_heatmap(writer, target02_output, "TestLatentVector", "target02",
                #                                (epoch - 1) * len(test_loader) + batch_idx)
                tensorboard.add_byol_latent_heatmap(writer, online01_output, target02_output, "TestLatentVector", "online01_vs_target02",
                                                    (epoch - 1) * len(test_loader) + batch_idx)
                tensorboard.add_byol_latent_heatmap(writer, online02_output, target01_output, "TestLatentVector", "online02_vs_target01",
                                                    (epoch - 1) * len(test_loader) + batch_idx)
